#include "statis2.h"
#include "ui_statis2.h"
#include <QString>
#include<qsqlquery.h>

statis2::statis2(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::statis2)
{
    ui->setupUi(this);
...
void statis2::make()
{
        ...
        q.prepare("SELECT COUNT(id) FROM evenements where type='chat' ");
        q.exec();
        q.first() ;
        chatt=(q.value(0).toInt());

        q.prepare("SELECT COUNT(id) FROM evenements where type='chien' ");
        q.exec();
        q.first() ;
