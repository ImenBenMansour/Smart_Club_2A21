...
*/
 ...
 /*bool Nourriture::update_rfid(QString id_N,QString code_rfid)
 {
     QSqlQuery qry;
     qry.prepare("UPDATE nourriture SET code_rfid= :code_rfid where id_N=:id_N");
     qry.bindValue(":id_N",id_N);
     qry.bindValue(":code_rfid",code_rfid);

     return qry.exec();
 }
 bool Nourriture::verify_rfid(QString code_rfid)
 {
     QSqlQuery qry;
     qry.prepare("SELECT * FROM nourriture WHERE code_rfid=:code_rfid");
     qry.bindValue(":code_rfid",code_rfid);
